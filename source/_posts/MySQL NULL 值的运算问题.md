...
最终执行的过程中，以上的 SQL 会忽略 `b` 列为 `NULL` 的记录，即上述 SQL 和下面的 SQL 其实不是等价的：

```sql
SELECT count(*) FROM `table` WHERE id = 1 AND (NOT (b = 'b' AND b IS NOT NULL));
```

从逻辑上说， `b == 'b'` 的非表述就是 `!(b == 'b')` ，这没毛病，这个问题的关键就是在于 MySQL 对于 `NULL` 值的处理上。
...
而 MySQL 中，0 或者 NULL 值被看作是 **false**，所以最开始的 SQL，如果遇到了 `NULL` 值，则 where 条件变为：
